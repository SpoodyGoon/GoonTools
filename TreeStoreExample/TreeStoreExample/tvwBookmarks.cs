...
namespace BookmarkSharpClient.TreeViews
{
	...
	public class tvwBookmarks : Gtk.TreeView
	{
		...
		public tvwBookmarks()
		{
			...
			Gtk.TreeViewColumn colIsLabel = new Gtk.TreeViewColumn ();
			colIsLabel.Visible = false;
			colIsLabel.Title = "IsLabel";
			...
			Gtk.CellRendererText cellIsLabel = new Gtk.CellRendererText ();
			cellIsLabel.Visible = false;
			colIsLabel.PackStart (cellIsLabel, true);
			...
			colIsLabel.AddAttribute(cellIsLabel, "text", 3);
		...
		{
			SqliteConnection sqlCN = new SqliteConnection("URI=file:TreeStoreExample.db3,version=3");
			SqliteCommand sqlCMD = new SqliteCommand("SELECT BookmarkID, Title, URL, IsFolder, ParentID, PathDepth FROM tb_Bookmarks ORDER BY PathDepth;", sqlCN);
			SqliteDataReader sqlReader = null;
			try
			{
				// clear the treestore to start fresh
				_ts.Clear();
				// declare the TreeIter
				Gtk.TreeIter iter;
				sqlCN.Open();
				sqlReader = sqlCMD.ExecuteReader();
				while(sqlReader.Read())
				{
					if(sqlReader["PathDepth"].ToString() == "0")
					{
						iter = _ts.AppendValues(sqlReader["BookmarkID"].ToString(), sqlReader["Title"].ToString(),sqlReader["URL"].ToString(),sqlReader["IsFolder"].ToString(),sqlReader["ParentID"].ToString());
					}
					else
					{
						iter = _ts.AppendValues((Gtk.TreeIter)GetParentIter(sqlReader["ParentID"].ToString()), sqlReader["BookmarkID"].ToString(), sqlReader["Title"].ToString(),sqlReader["URL"].ToString(),sqlReader["IsFolder"].ToString(),sqlReader["ParentID"].ToString());
					}
					AddParentIter(sqlReader["BookmarkID"].ToString(), iter);
				}
...
//			dr["IsLabel"] = model.GetValue(iter,3).ToString();
//			if(path.Depth > 0)
//			{

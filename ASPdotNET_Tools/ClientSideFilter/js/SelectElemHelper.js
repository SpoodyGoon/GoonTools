/*
	...
	It is intend to be so simple/stupid that it will function in any browser
	It doesn't do anything fancy it just is a bunch of loops
*/
...
function RemoveOptionByValue(SelectElem, SelectValue)
{
	if (IsSelect(SelectElem)) 
	{
		...
		for (var i = 0; i < SelectElem.options.length; i++) 
		{			
			if (SelectElem.options[i].value.toLowerCase() == SelectValue.toLowerCase()) 
			{				
				SelectElem.remove(i);
			}
		}			
...
function RemoveOptionByText(SelectElem, SelectText)
{
	try
	{
		if (IsSelect(SelectElem)) 
		{
			...
			for (var i = 0; i < SelectElem.options.length; i++) 
			{			
				if (SelectElem.options[i].text.toLowerCase() == SelectText.toLowerCase()) 
				{				
					SelectElem.remove(i);
				}
			}			
...
function MultiSelectGetValue(SelectElem)
{
...
// gets all the selected options in a select/list element
// and returns them in an array
function MultiSelectGetText(SelectElem)
{
	var SelectArray = null;
	if(IsMultiSelect(SelectElem))
	{		
		...
		if(SelectElem.selectedIndex != -1)
		{
			for(var i = 0; i < SelectElem.options.length; i ++)
			{
				if(SelectElem.options[i].selected)
				{
					SelectArray.push(SelectElem.options[i].text);
				}
			}
		}
	}
...
// gets all the selected options in a select/list element
// and returns them in an 2 dementional array
// [value][text]
function MultiSelectGetValueText(SelectElem)
{
	var SelectArray = null;
	var tmpArray = null;
	if(IsMultiSelect(SelectElem))
	{		
		...
		if(SelectElem.selectedIndex != -1)
		{
			for(var i = 0; i < SelectElem.options.length; i ++)
			{
				if(SelectElem.options[i].selected)
				{				
					tmpArray = new Array(SelectElem.options[i].value, SelectElem.options[i].text); 	
					SelectArray.push(tmpArray);
				}
			}
		}
	}
